...
export default async function ActivityPage() {
  ...
  const { data: logs } = await supabase
    .from('activity_logs')
    .select(
      'id, restaurant_id, action_type, action_details, performed_by, created_at, restaurants(restaurant_name)'
    )
    ...
    <div style={{ padding: '32px 40px 60px', maxWidth: 1440, margin: '0 auto' }}>
      ...
      >
        <div style={{ padding: '20px 24px' }}>
          {logs && logs.length > 0 ? (
            logs.map((a: any) => {
              const ic = iconMap[a.action_type] || {
                cls: 'activity-icon-note',
                emoji: '•',
              }
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      <strong>{a.restaurants?.restaurant_name || 'Restaurant'}</strong> —{' '}
                      {a.action_details}
                    </div>
                    <div
                      style={{
